...
@app.get("/food-stands", response_model=list[models_equipo4.FoodStand])
def list_food_stands(session: Session = Depends(get_session)):
    statement = select(models_equipo4.FoodStand)
    results = session.exec(statement)
    return results.all()


@app.post("/food-stands", response_model=models_equipo4.FoodStand)
def create_food_stand(
    food_stand: models_equipo4.FoodStand,
